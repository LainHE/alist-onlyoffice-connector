...
整个过程使用了alist、onlyoffice (documentserver)、nginx、php，所有服务与中间件均通过docker方式部署，其中 nginx、php 网络模式为 host，alist、onlyoffice 网络模式为bridge 但增加了 --add-host=host.docker.internal:host-gateway 用于保证链接。  
个人专注于本地局域网使用，请自行更改代码中所有xxxx、x.x.x.x:xxxx，x.x.x.x:yyyy等部分，如也需局域网使用，请：  
需要进入onlyoffice容器中，修改配置文件/etc/onlyoffice/documentserver/default.json，注意以下两个内容：  

"rejectUnauthorized": false  
request-filtering-agent 中的 allowPrivateIPAddress、allowMetaIPAddress 均为true  
第1点是关闭ssl，第2点是允许使用IP访问，默认是只使用域名访问。  

相较于参考的Github讨论部分，html新增了css样式，保证了没有双滚动条，并移除了内外边框。  
所有参考在本篇底部。  

### 预览模式  
预览模式不需要php回调，只需要nginx即可，如不知具体操作方法，请见参考：[Pwolf私人博客 - 局域网内部署alist和onlyoffice](https://blog.pwolf.site/2023/09/15/%E5%B1%80%E5%9F%9F%E7%BD%91%E5%86%85%E9%83%A8%E7%BD%B2alist%E5%92%8Conlyoffice/)  
...
server{
    ...
    location ~ \.php$ {
        fastcgi_pass host.docker.internal:xxxx;
...
...
{
	"doc,docx,xls,xlsx,ppt,pptx": {
		"Onlyoffice View":"http://x.x.x.x:xxxx/view.html?src=$e_url",
		"Onlyoffice Edit":"http://x.x.x.x:xxxx/edit.html?src=$e_url"
	},
...
```
### 参考
[官方文档 - alist api - 获取token](https://alist-v3.apifox.cn/api-128101241)
[官方文档 - alist api - 流式上传文件](https://alist-v3.apifox.cn/api-128101260)
[官方文档 - onlyoffice 回调程序](https://api.onlyoffice.com/docs/docs-api/usage-api/callback-handler/)
[官方文档 - onlyoffice 编辑模式](https://api.onlyoffice.com/docs/docs-api/usage-api/config/editor/)
[Github讨论#3255 - 预览模式](https://github.com/AlistGo/alist/discussions/3255)
[Github讨论#3899 - 编辑模式](https://github.com/AlistGo/alist/discussions/3899)
[Pwolf私人博客 - 局域网内部署alist和onlyoffice](https://blog.pwolf.site/2023/09/15/%E5%B1%80%E5%9F%9F%E7%BD%91%E5%86%85%E9%83%A8%E7%BD%B2alist%E5%92%8Conlyoffice/)
